...
public class VistaListaExpedientesActivos extends CustomComponent {
	...
	private final String consultaInicial = "SELECT codigo, numexpediente, instructorasig,fechaingreso, clasificacion, estadoDelPlazo "
											+ "FROM expediente "
											+ "WHERE archivado = false";
	private final String consultaPlazo = "SELECT codigo, numexpediente, instructorasig, fechaingreso, clasificacion, estadoDelPlazo FROM expediente WHERE archivado = false AND estadoDelPlazo != 'A tiempo'";
	...
	public VistaListaExpedientesActivos() {
		...
		checkboxAtrasados.addListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(ValueChangeEvent event) {
                ...
                String consulta;
                	tableExpAct.removeAllItems();
        			if (filtro.isEmpty()) {
        				if (checkboxAtrasados.getValue() == false) {
        					llenarTabla(consultaInicial);
        				}
        				else {
        					llenarTabla(consultaPlazo);
        				}
        			}
        			else {
        				if (checkboxAtrasados.getValue() == false) {
        					...
        					consulta = consultaPlazo + " AND " + columna + " LIKE " + "'%" + filtro + "%'";
        					llenarTabla(consulta);
        				}
        			}
            	}
		...
		tableExpAct.setCellStyleGenerator(new Table.CellStyleGenerator() {
		      ...
		      public String getStyle(Table source, Object itemId, Object propertyId) {
		        if (propertyId == null) {
		          // Styling for row
		          String estado = tableExpAct.getItem(itemId).getItemProperty("estadoDelPlazo").getValue().toString();
		          if (estado.compareTo("Cerca de vencer") == 0) {
		            return "highlight-yellow";
		          } else if (estado.compareTo("Vencido") == 0) {
		            return "highlight-red";
		          } else if (estado.compareTo("A tiempo") == 0) {
		...
		buttonReporte.addClickListener(new ClickListener(){
			...
			public void buttonClick (ClickEvent evento){
				...
				pdf(VaadinService.getCurrent().getBaseDirectory().getAbsolutePath().replace("\\", "/") + "/WEB-INF/reporte/ExpedientesActivos.pdf");
			}
		});
	...
	public void setVista() {
		...
		Col col7 = row4.addCol(ColMod.XS_12, ColMod.SM_3, ColOffsetMod.MD_OFFSET_1, MarginMod.TOP5);
		col7.addComponent(buttonReporte);
		Col col8 = row4.addCol(ColMod.XS_12, ColMod.SM_3, ColOffsetMod.SM_OFFSET_1, MarginMod.TOP5);
		col8.addComponent(buttonExcel);
		Col col5 = row4.addCol(ColMod.XS_12, ColMod.SM_3, ColOffsetMod.SM_OFFSET_1, MarginMod.TOP5);
	...
	private AbsoluteLayout buildMainLayout() {
		...
		mainLayout.addComponent(buttonReporte, "top:510.0px;left:260.0px;");
		
		// buttonExcel
		buttonExcel = new Button();
		buttonExcel.setCaption("Generar csv");
		buttonExcel.setImmediate(false);
		buttonExcel.setWidth("-1px");
		buttonExcel.setHeight("-1px");
		mainLayout.addComponent(buttonExcel, "top:506.0px;left:400.0px;");
